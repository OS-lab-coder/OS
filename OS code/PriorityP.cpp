...
int main()
{
    ...
    cin>>n;
    cout<<endl;
    int at[n], bt[n], ct[n], tat[n], wt[n], Pid[n], btc[n], rt[n], fo[n], priority[n];
    for(i=0; i<n; i++)
        fo[i] = 0;
    for(i=0; i<n; i++)
    {
    ...
    int time = at[0], pid, complete, b, index = 0, h = 0;
    while(1)
    {
        b = 0;
        for(i=0; i<n; i++)
    ...
    display(q,&f,&r);
    cout<<endl;
    cout<<endl<<"Pid\tAT\tBT\tPriority\tCT\tTAT\tWT\tRT"<<endl;
    for(i=0; i<n; i++)
    {
        cout<<Pid[i]<<"\t"<<at[i]<<"\t"<<btc[i]<<"\t"<<priority[i]<<"\t\t"<<ct[i]<<"\t"<<tat[i]<<"\t"<<wt[i]<<"\t"<<rt[i]<<endl;
    }
